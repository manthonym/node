...
describe 'HTTP REST API', ->
  ...
  it 'post and get a metric', (next) ->
    request.post 'http://localhost:1234/metric/2.json', [
    ...
    ,
      ...
      request.get 'http://localhost:1234/metric/2.json', (err, res, body) ->
  ...
  it 'remove a metric', (next) ->
    ...
    ,
      ...
      return next err if err
      return new Error "Invalid response code: #{res.statusCode}" unless res.statusCode is 200
      request.delete 'http://localhost:1234/metric/2.json', (err, res, body) ->
        return next err if err
        return new Error "Invalid response code: #{res.statusCode}" unless res.statusCode is 200
        request.get 'http://localhost:1234/metric/2.json', (err, res, body) ->
          return next err if err
