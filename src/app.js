...
(function() {
  var app, express, hashes, http, metric_get, stylus, user;
  ...
  metric_get = function(req, res, next) {
    return metrics.access(req.cookies.remember, req.params.id, function(err, values) {
      if (err) {
        return next(err);
      }
      if (values[0].username === req.cookies.remember) {
  ...
  app.post('/user/connect', function(req, res) {
    ...
    return user.log(req.body.username, req.body.password, function(err, values) {
      ...
      if (values.length === 1) {
        console.log('yo');
        minute = 30 * 60 * 1000;
        res.cookie('remember', req.body.username, {
          maxAge: minute,
          httpOnly: false
        });
        ...
        if (values[0].password === sha256.hex(req.body.password)) {
          return res.render('index', {
            name: values[0].login
          });
        }
  ...
  app.post('/data/save', function(req, res, next) {
    ...
    metrics.link(req.cookies.remember, req.body.id, function(err) {
      if (err) {
        return next(err);
      }
    });
